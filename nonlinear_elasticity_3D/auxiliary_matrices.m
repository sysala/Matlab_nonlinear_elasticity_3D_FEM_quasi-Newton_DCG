...
function [B,B_V,B_grad,K_V,K_elast,WEIGHT]=auxiliary_matrices(ELEM,COORD,...
                                        bulk,shear,DHatP1,DHatP2,DHatP3,WF)
 
% =========================================================================
%
%  This function assembles auxiliary arrays and matrices which are 
%  important for Newton's and quasi-Newton's solvers
%
%  input data:
...
%    K_elast- the elastic stiffness matrix, size(K_elast)=(3*n_n,3*n_n)
...
%
% matrix representing gradient of U, size(B_grad)=(9*n_int,3*n_n)
%

  % values of the matrix representing grad U
  n_b = 27*n_p ;
  vB=zeros(n_b,n_int);        % size(vB)=(27*n_p,n_int)
  vB(1:27:n_b-26,:)=DPhi1; vB(11:27:n_b-16,:)=DPhi1; vB(21:27:n_b-6,:)=DPhi1;
  vB(4:27:n_b-23,:)=DPhi2; vB(14:27:n_b-13,:)=DPhi2; vB(24:27:n_b-3,:)=DPhi2;
  vB(7:27:n_b-20,:)=DPhi3; vB(17:27:n_b-10,:)=DPhi3; vB(27:27:n_b  ,:)=DPhi3;
  
  % i-th and j-th indices of B: size(iB)=size(jB)=(27*n_p,n_int)
  AUX9=reshape(1:9*n_int,9,n_int);
  iB=repmat(AUX9,3*n_p,1);  
  
  AUX1 = [1;1;1]*(1:n_p);     % size(AUX1)=(3,n_p)
  AUX2 = [2;1;0]*ones(1,n_p); % size(AUX2)=(3,n_p)
  AUX3=3*ELEM((AUX1(:))',:)-kron(ones(1,n_e),AUX2(:));
                              % size(AUX3)=(3*n_p,n_e)
  jB=kron(AUX3,ones(9,n_q));
  
  % the sparse grad-displacement matrix B
  B_grad=sparse(iB(:),jB(:),vB(:), 9*n_int,3*n_n);  
 
%
% matrix representing volumetric strain, size(B_D)=(n_int,3*n_n)
%  
  B_V=B(1:6:6*n_int-5,:)+B(2:6:6*n_int-4,:)+B(3:6:6*n_int-3,:); 

%
% volumetric part of the elastic stiffness matrix: size(K_V)=(3*n_n,3*n_n)
%   

  D_V = sparse( 1:n_int,1:n_int,WEIGHT.*bulk) ;
  K_V = B_V'*D_V*B_V ;    

%
% the elastic stiffness matrix: size(K_elast)=(3*n_n,3*n_n)
%   
