...
function [S,DS,m,M]=constitutive_problem_quasi3(E,shear,bulk,model_type)
...
% 1,2,...,n_int. These auxiliary arrays are related to Preconditioner 3
...
%           (It represent the function \mu_0.) size(DS)=(1,n_int)
...
%
  kappa=4; % estimate of the Korn constant
  mu_1=mu+2*Dmu;
  mu_max=max(mu,mu_1);
  mu_min=min(mu,mu_1);
  m=min(mu_min./mu_delta)/kappa;
