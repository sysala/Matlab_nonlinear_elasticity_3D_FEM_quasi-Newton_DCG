...
function [coord,elem,surf,neumann,Q]=mesh_P2(density,size_xy,size_z)
...
%
  ...
  N_x = size_xy*density;      % number of segments in x direction
  N_y = N_x;                  % number of segments in y direction
  N_z = size_z*density;       % number of segments in z-direction
  % 
...
%
  ...
  % nonhomogeneous Neumann boundary conditions on a part of Face 5
  N1_x = density;          % number of segments in x direction
  C_s=zeros(2*N1_x+1,2*N_z+1);
  C_s(:,:)=C(1:2*N1_x+1,end,:);
  V1_s=false(2*N1_x+1,2*N_z+1);   V1_s(1:2:(2*N1_x-1),1:2:(2*N_z-1))=1;
  V2_s=false(2*N1_x+1,2*N_z+1);   V2_s(3:2:(2*N1_x+1),1:2:(2*N_z-1))=1;
  V3_s=false(2*N1_x+1,2*N_z+1);   V3_s(3:2:(2*N1_x+1),3:2:(2*N_z+1))=1;
  V4_s=false(2*N1_x+1,2*N_z+1);   V4_s(1:2:(2*N1_x-1),3:2:(2*N_z+1))=1;
  V12_s=false(2*N1_x+1,2*N_z+1);  V12_s(2:2:(2*N1_x  ),1:2:(2*N_z-1))=1;
  V14_s=false(2*N1_x+1,2*N_z+1);  V14_s(1:2:(2*N1_x-1),2:2:(2*N_z  ))=1;
  V23_s=false(2*N1_x+1,2*N_z+1);  V23_s(3:2:(2*N1_x+1),2:2:(2*N_z  ))=1;
  V13_s=false(2*N1_x+1,2*N_z+1);  V13_s(2:2:(2*N1_x  ),2:2:(2*N_z  ))=1;
  V34_s=false(2*N1_x+1,2*N_z+1);  V34_s(2:2:(2*N1_x  ),3:2:(2*N_z+1))=1;
  aux_surf=[C_s(V4_s)'; C_s(V1_s)'; C_s(V3_s)'; C_s(V13_s)'; C_s(V34_s)'; C_s(V14_s)';
            C_s(V2_s)'; C_s(V3_s)'; C_s(V1_s)'; C_s(V13_s)'; C_s(V12_s)'; C_s(V23_s)'; ];
  neumann=reshape(aux_surf,6,2*N1_x*N_z);          
