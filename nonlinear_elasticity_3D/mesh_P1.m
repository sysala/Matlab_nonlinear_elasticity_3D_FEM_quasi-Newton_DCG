...
function [coord,elem,surf,neumann,Q]=mesh_P1(density,size_xy,size_z)
...
%
  ...
  C_s=zeros(N1_x+1,N_z+1);
  C_s(:,:)=C(1:N1_x+1,end,:);  
  V1_s=false(N1_x+1,N_z+1);  V1_s(1:N1_x    ,1:N_z)=1;
  V2_s=false(N1_x+1,N_z+1);  V2_s(2:(N1_x+1),1:N_z)=1;
  V3_s=false(N1_x+1,N_z+1);  V3_s(2:(N1_x+1),2:(N_z+1))=1;
  V4_s=false(N1_x+1,N_z+1);  V4_s(1:N1_x    ,2:(N_z+1))=1;  
  aux_surf=[C_s(V4_s)'; C_s(V1_s)'; C_s(V3_s)';
            C_s(V2_s)'; C_s(V3_s)'; C_s(V1_s)' ];
  neumann=reshape(aux_surf,3,2*N1_x*N_z);       
